...
static void gimp_tools_register(GType tool_type, GType tool_options_type,
  GimpToolOptionsGUIFunc options_gui_func, GimpContextPropMask context_props,
  const gchar *identifier, const gchar *blurb, const gchar *help, const gchar *menu_path,
  const gchar *menu_accel, const gchar *help_domain, const gchar *help_data,
  const gchar *stock_id, gpointer data);
...
void gimp_tools_init(Gimp *gimp)
{
  ...
  for(list = GIMP_LIST(gimp->tool_info_list)->list; list; list = g_list_next(list))
  {
...
void gimp_tools_exit(Gimp *gimp)
{
  ...
  default_order = g_object_get_data(G_OBJECT(gimp), "gimp-tools-default-order");
...
void gimp_tools_restore(Gimp *gimp)
{
  ...
  if(gimp_config_deserialize_file(GIMP_CONFIG(gimp_list), filename, NULL, NULL))
  {
    ...
    for(list = GIMP_LIST(gimp_list)->list, i = 0; list; list = g_list_next(list), i++)
    {
      const gchar *name;
      GimpObject  *object;
      name = gimp_object_get_name(list->data);
      object = gimp_container_get_child_by_name(gimp->tool_info_list, name);
      if(object)
      {
        g_object_set(object, "visible", GIMP_TOOL_INFO(list->data)->visible, NULL);
        gimp_container_reorder(gimp->tool_info_list, object, MIN(i, n - 1));
      }
    }
  }
  ...
  {
    gimp_message(gimp, NULL, GIMP_MESSAGE_WARNING, "%s", error->message);
    g_clear_error(&error);
  }
  ...
  for(list = GIMP_LIST(gimp->tool_info_list)->list; list; list = g_list_next(list))
  {
    ...
    // (see bug #541586).
    gimp_context_copy_properties(gimp_get_user_context(gimp), GIMP_CONTEXT(tool_info->tool_options),
      GIMP_CONTEXT_ALL_PROPS_MASK &~ tool_info->context_props);

    gimp_tool_options_deserialize(tool_info->tool_options, NULL);
    options_gui_func = g_object_get_data(G_OBJECT(tool_info), "gimp-tool-options-gui-func");
    ...
    else
    {
      ...
      gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_STYLE, PANGO_STYLE_ITALIC, -1);
      gtk_box_pack_start(GTK_BOX (options_gui), label, FALSE, FALSE, 6);
      gtk_widget_show(label);
    }

    g_object_set_data(G_OBJECT(tool_info->tool_options), "gimp-tool-options-gui", options_gui);

    if(tool_info->presets) gimp_tool_presets_load(tool_info->presets, NULL);
  }
}

void gimp_tools_save(Gimp *gimp, gboolean save_tool_options, gboolean always_save)
{
  gchar *filename;
  ...
  gimp_config_serialize_to_file(GIMP_CONFIG(gimp->tool_info_list), filename,
    "GIMP toolrc", "end of toolrc", NULL, NULL);
  g_free(filename);
}

gboolean gimp_tools_clear(Gimp *gimp, GError **error)
{
  GList    *list;
  ...
  for(list = GIMP_LIST(gimp->tool_info_list)->list; list && success; list = g_list_next(list))
  {
    GimpToolInfo *tool_info = GIMP_TOOL_INFO(list->data);
    success = gimp_tool_options_delete(tool_info->tool_options, NULL);
  }

  if(success) success = gimp_contexts_clear(gimp, error);
  if(success) tool_options_deleted = TRUE;
  return success;
}
...
GList* gimp_tools_get_default_order(Gimp *gimp)
{
  ...
  return g_object_get_data(G_OBJECT(gimp), "gimp-tools-default-order");
}
  ...
  const gchar *stock_id, gpointer data)
{
  ...
  g_return_if_fail(tool_options_type == G_TYPE_NONE || g_type_is_a(tool_options_type, GIMP_TYPE_TOOL_OPTIONS));
  ...
  tool_info = gimp_tool_info_new(gimp, tool_type, tool_options_type, context_props, identifier,
    blurb, help, menu_path, menu_accel, help_domain, help_data, paint_core_name, stock_id);

  visible = (!g_type_is_a(tool_type, GIMP_TYPE_IMAGE_MAP_TOOL));

  g_object_set(tool_info, "visible", visible, NULL);
  g_object_set_data(G_OBJECT(tool_info), "gimp-tool-default-visible", GINT_TO_POINTER(visible));
  g_object_set_data(G_OBJECT(tool_info), "gimp-tool-options-gui-func", options_gui_func);
  gimp_container_add(gimp->tool_info_list, GIMP_OBJECT(tool_info));
  g_object_unref(tool_info);
  if(tool_type == GIMP_TYPE_PAINTBRUSH_TOOL) gimp_tool_info_set_standard(gimp, tool_info);
}
