...
struct _GimpTransformTool
{
  ...
  gint            ngx, ngy;        // number of grid lines in original x and y directions
                                    
  gdouble        *grid_coords;     // x and y coordinates of the grid endpoints
                                   // (a total of (ngx + ngy)*2 coordinate pairs)
...
struct _GimpTransformToolClass
{
  ...
  void (*prepare)(GimpTransformTool *tool, GimpDisplay *display);
  void (*motion)(GimpTransformTool *tool, GimpDisplay *display);
  void (*recalc)(GimpTransformTool *tool, GimpDisplay *display);
  TileManager* (*transform)(GimpTransformTool *tool, GimpItem *item, gboolean mask_empty, GimpDisplay *display);
};
