...
static gboolean gimp_rotate_tool_key_press(GimpTool *tool, GdkEventKey *kevent, GimpDisplay *display);
...
static void gimp_rotate_tool_prepare(GimpTransformTool *tr_tool, GimpDisplay *display);
static void gimp_rotate_tool_motion(GimpTransformTool *tr_tool, GimpDisplay *display);
static void gimp_rotate_tool_recalc(GimpTransformTool *tr_tool, GimpDisplay *display);
static void rotate_angle_changed(GtkAdjustment *adj, GimpTransformTool *tr_tool);
static void rotate_center_changed(GtkWidget *entry, GimpTransformTool *tr_tool);
...
{
  (*callback)(GIMP_TYPE_ROTATE_TOOL, GIMP_TYPE_TRANSFORM_OPTIONS, gimp_transform_options_gui,
    GIMP_CONTEXT_BACKGROUND_MASK, "gimp-rotate-tool", _("Rotate"),
    _("Rotate Tool: Rotate the layer, selection or path"), N_("_Rotate"), "<shift>R", NULL,
    GIMP_HELP_TOOL_ROTATE, GIMP_STOCK_TOOL_ROTATE, data);
}
...
static void gimp_rotate_tool_dialog(GimpTransformTool *tr_tool)
{
  ...
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(tr_tool->dialog)->vbox), table, FALSE, FALSE, 0);
  gtk_widget_show(table);

  button = gimp_spin_button_new(&rotate->angle_adj, 0, -180, 180, 0.1, 15, 0, 2, 2);
  gtk_spin_button_set_wrap(GTK_SPIN_BUTTON(button), TRUE);
  gtk_entry_set_width_chars(GTK_ENTRY(button), SB_WIDTH);
  gimp_table_attach_aligned(GTK_TABLE(table), 0, 0, _("_Angle:"), 0.0, 0.5, button, 1, TRUE);
  rotate->angle_spin_button = button;

  g_signal_connect(rotate->angle_adj, "value-changed", G_CALLBACK(rotate_angle_changed), tr_tool);

  scale = gtk_hscale_new(GTK_ADJUSTMENT(rotate->angle_adj));
  gtk_scale_set_draw_value(GTK_SCALE(scale), FALSE);
  gtk_table_attach(GTK_TABLE(table), scale, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0);
  gtk_widget_show(scale);

  button = gimp_spin_button_new(&adj, 0, -1, 1, 1, 10, 0, 1, 2);
  gtk_entry_set_width_chars(GTK_ENTRY(button), SB_WIDTH);
  gimp_table_attach_aligned(GTK_TABLE(table), 0, 2, _("Center _X:"), 0.0, 0.5, button, 1, TRUE);

  rotate->sizeentry = gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE, SB_WIDTH,
    GIMP_SIZE_ENTRY_UPDATE_SIZE);
  gimp_size_entry_add_field(GIMP_SIZE_ENTRY(rotate->sizeentry), GTK_SPIN_BUTTON(button), NULL);
  gimp_size_entry_set_pixel_digits(GIMP_SIZE_ENTRY(rotate->sizeentry), 2);
  gimp_table_attach_aligned(GTK_TABLE(table), 0, 3, _("Center _Y:"), 0.0, 0.5, rotate->sizeentry, 1, TRUE);

  g_signal_connect(rotate->sizeentry, "value-changed", G_CALLBACK(rotate_center_changed), tr_tool);
}
...
static void gimp_rotate_tool_dialog_update(GimpTransformTool *tr_tool)
{
  ...
  g_signal_handlers_block_by_func(rotate->sizeentry, rotate_center_changed, tr_tool);

  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(rotate->sizeentry), 0, tr_tool->trans_info[CENTER_X]);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(rotate->sizeentry), 1, tr_tool->trans_info[CENTER_Y]);

  g_signal_handlers_unblock_by_func(rotate->sizeentry, rotate_center_changed, tr_tool);
}
...
static void gimp_rotate_tool_prepare(GimpTransformTool *tr_tool, GimpDisplay *display)
{
  ...
  gimp_size_entry_set_unit(GIMP_SIZE_ENTRY(rotate->sizeentry), GIMP_DISPLAY_SHELL(display->shell)->unit);

  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(rotate->sizeentry), 0, xres, FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(rotate->sizeentry), 1, yres, FALSE);

  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(rotate->sizeentry), 0,
    -65536, 65536 + gimp_image_get_width(display->image));
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(rotate->sizeentry), 1,
    -65536, 65536 + gimp_image_get_height(display->image));

  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(rotate->sizeentry), 0, tr_tool->x1, tr_tool->x2);
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(rotate->sizeentry), 1, tr_tool->y1, tr_tool->y2);
...
static void gimp_rotate_tool_motion(GimpTransformTool *tr_tool, GimpDisplay *display)
{
  ...
  {
    tr_tool->trans_info[ANGLE] = FIFTEEN_DEG*(gint)((tr_tool->trans_info[REAL_ANGLE] + FIFTEEN_DEG/2.0)/FIFTEEN_DEG);
  }
  else
  {
...
static void gimp_rotate_tool_recalc(GimpTransformTool *tr_tool, GimpDisplay *display)
{
  ...
  gimp_transform_matrix_rotate_center(&tr_tool->transform, tr_tool->cx, tr_tool->cy, tr_tool->trans_info[ANGLE]);
}
...
static void rotate_center_changed(GtkWidget *widget, GimpTransformTool *tr_tool)
{
  ...
  if((cx != tr_tool->trans_info[CENTER_X]) || (cy != tr_tool->trans_info[CENTER_Y]))
  {
    gimp_draw_tool_pause(GIMP_DRAW_TOOL(tr_tool));
