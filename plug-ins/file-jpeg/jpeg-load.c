...
static void jpeg_load_resolution(gint32 image_ID, struct jpeg_decompress_struct *cinfo);
static void jpeg_load_sanitize_comment(gchar *comment);
static gpointer jpeg_load_cmyk_transform(guint8 *profile_data, gsize profile_len);
static void jpeg_load_cmyk_to_rgb(guchar *buf, glong pixels, gpointer transform);

GimpDrawable    *drawable_global;
gint32 volatile  preview_image_ID;
gint32           preview_layer_ID;

gint32 load_image(const gchar *filename, GimpRunMode runmode, gboolean preview, GError **error)
{
  ...
  if(setjmp(jerr.setjmp_buffer))
  {
    ...
    if(image_ID != -1 && !preview) gimp_image_delete(image_ID);
    if(preview) destroy_preview();
  ...
  //fallthrough
  default:
    g_message("Don't know how to load JPEG images with %d color channels, using colorspace %d (%d).",
      cinfo.output_components, cinfo.out_color_space, cinfo.jpeg_color_space);
    return -1;
    break;
  }
  ...
  if(preview)
  {
    preview_layer_ID = gimp_layer_new(preview_image_ID, _("JPEG preview"), cinfo.output_width,
      cinfo.output_height, layer_type, 100, GIMP_NORMAL_MODE);
    layer_ID = preview_layer_ID;
  }
  else
  {
    layer_ID = gimp_layer_new(image_ID, _("Background"), cinfo.output_width, cinfo.output_height,
  ...
  if(!preview)
  {
    ...
    for(marker = cinfo.marker_list; marker; marker = marker->next)
    {
      ...
      else if((marker->marker == JPEG_APP0 + 1) && (len > sizeof(JPEG_APP_HEADER_EXIF) + 8) &&
...
#endif
#ifdef HAVE_EXIF
        if(! exif_data) exif_data = exif_data_new();
    ...
    if(comment_buffer && comment_buffer->len)
    {
      ...
      parasite = gimp_parasite_new("gimp-comment", GIMP_PARASITE_PERSISTENT,
        strlen(comment_buffer->str) + 1, comment_buffer->str);
    ...
    for(marker = cinfo.marker_list; marker; marker = marker->next)
    {
      ...
      if((marker->marker == JPEG_APP0 + 1) && (len > sizeof(JPEG_APP_HEADER_XMP) + 20) &&
...
#endif
        xmp_packet = g_strndup(data + sizeof(JPEG_APP_HEADER_XMP), len - sizeof(JPEG_APP_HEADER_XMP));

        // FIXME: running this through the PDB is not very efficient
        return_vals = gimp_run_procedure("plug-in-metadata-decode-xmp", &nreturn_vals,
          GIMP_PDB_IMAGE, image_ID, GIMP_PDB_STRING, xmp_packet, GIMP_PDB_END);
    ...
    else if(profile) // don't attach the profile if we are transforming
    {
      GimpParasite *parasite = gimp_parasite_new("icc-profile",
        GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE, profile_size, profile);
      gimp_image_parasite_attach(image_ID, parasite);
      gimp_parasite_free(parasite);
    }
  ...
  while(cinfo.output_scanline < cinfo.output_height)
  {
    ...
    if(cinfo.out_color_space == JCS_CMYK)
      jpeg_load_cmyk_to_rgb(buf, drawable->width*scanlines, cmyk_transform);

    gimp_pixel_rgn_set_rect(&pixel_rgn, buf, 0, start, drawable->width, scanlines);

    if(!preview && (cinfo.output_scanline % 32) == 0)
      gimp_progress_update((gdouble)cinfo.output_scanline/(gdouble)cinfo.output_height);
  }
...
#ifdef HAVE_LCMS
  if(cmyk_transform) cmsDeleteTransform(cmyk_transform);
#endif
...
static void jpeg_load_resolution(gint32 image_ID, struct jpeg_decompress_struct *cinfo)
{
  if(cinfo->saw_JFIF_marker && cinfo->X_density != 0 && cinfo->Y_density != 0)
  {
    ...
    {
    case 0: // unknown -> set the aspect ratio but use the default image resolution
    ...
    case 2: // dots per cm
      ...
      break;
    default:
      g_message("Unknown density unit %d, assuming dots per inch.", cinfo->density_unit);
      break;
    }
...
gint32 load_thumbnail_image(const gchar *filename, gint *width, gint *height, GError **error)
{
  gint32 volatile  image_ID;
  ExifData        *exif_data;
  GimpPixelRgn     pixel_rgn;
  ...
  layer_ID = gimp_layer_new(image_ID, _("Background"), cinfo.output_width,
    cinfo.output_height, layer_type, 100, GIMP_NORMAL_MODE);
  ...
  // At this point you may want to check to see whether any corrupt-data warnings
  // occurred (test whether jerr.num_warnings is nonzero).
  gimp_image_add_layer(image_ID, layer_ID, 0);

  // NOW to get the dimensions of the actual image to return the calling app
  cinfo.err = jpeg_std_error(&jerr.pub);
  jerr.pub.error_exit = my_error_exit;
  jerr.pub.output_message = my_output_message;
  ...
  if(setjmp(jerr.setjmp_buffer))
  {
    ...
    if(image_ID != -1) gimp_image_delete(image_ID);
...
static gpointer jpeg_load_cmyk_transform(guint8 *profile_data, gsize profile_len)
{
  ...
  if(config->display_intent == GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC)
  {
    flags |= cmsFLAGS_BLACKPOINTCOMPENSATION;
  }

  transform = cmsCreateTransform(cmyk_profile, TYPE_CMYK_8_REV, rgb_profile, TYPE_RGB_8,
    config->display_intent, flags);
