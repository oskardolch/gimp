...
typedef struct _DicomInfo
{
  ...
  gint maxval; // For 16 and 24 bit image files, the max value which we need to normalize to
...
static gboolean save_image(const gchar *filename, gint32 image_ID, gint32 drawable_ID, GError **error);
static void dicom_loader(guint8 *pix_buf, DicomInfo *info, GimpPixelRgn *pixel_rgn);
static void guess_and_set_endian2(guint16 *buf16, gint length);
static void toggle_endian2(guint16 *buf16, gint length);
static void add_tag_pointer(GByteArray *group_stream, gint group, gint element, const gchar *value_rep,
  const guint8 *data, gint length);
static void add_tag_string(GByteArray *group_stream, gint group, gint element, const gchar *value_rep, const gchar *s);
static void add_tag_int(GByteArray *group_stream, gint group, gint element, const gchar *value_rep, gint value);
static gboolean write_group_to_file(FILE *DICOM, gint group, GByteArray *group_stream);
...
static void query(void)
{
  ...
  gimp_install_procedure(LOAD_PROC, "loads files of the dicom file format",
    ...
    "2003",
    N_("DICOM image"), NULL, GIMP_PLUGIN, G_N_ELEMENTS(load_args), G_N_ELEMENTS(load_return_vals), load_args, load_return_vals);

  gimp_register_file_handler_mime(LOAD_PROC, "image/x-dcm");
  gimp_register_magic_load_handler(LOAD_PROC, "dcm,dicom", "", "128,string,DICM");

  gimp_install_procedure(SAVE_PROC, "Save file in the DICOM file format",
    ...
    "RGB, GRAY", GIMP_PLUGIN, G_N_ELEMENTS(save_args), 0, save_args, NULL);
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  *return_vals  = values;
  ...
  else if(strcmp(name, SAVE_PROC) == 0)
  {
    ...
    if(export == GIMP_EXPORT_EXPORT) gimp_image_delete (image_ID);
  }
  else
  {
...
static gint32 load_image(const gchar *filename, GError **error)
{
  ...
  if(!DICOM)
  {
    g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
      _("Could not open '%s' for reading: %s"), gimp_filename_to_utf8(filename), g_strerror(errno));
    return -1;
  }
  ...
  {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("'%s' is not a DICOM file."),
      gimp_filename_to_utf8(filename));
    return -1;
  }
  ...
  if(fread(&group_word, 1, 2, DICOM) == 0) break;
  ...
  if(// Always need lookup for implicit encoding
    ...
    // I found this in one of Ednas images. It seems like a bug...
    && !(value_rep[0] == ' ' && value_rep[1]))
  )
  {
    // Look up type from the dictionary. At the time we dont support this option...
    ...
    // For implicit value_values the length is always four bytes, so we need to read another two.
    fread(&element_length_chars[2], 1, 2, DICOM);

    // Now cast to integer and insert into element_length
    element_length = g_ntohl(GUINT32_SWAP_LE_BE(*((gint*)element_length_chars)));
  }
  // Binary value reps are OB, OW, SQ or UN
  else if(strncmp (value_rep, "OB", 2) == 0 || strncmp (value_rep, "OW", 2) == 0
    || strncmp (value_rep, "SQ", 2) == 0 || strncmp (value_rep, "UN", 2) == 0)
  {
  ...
  if(element_length == 0xffffffff) continue;

  // Sequence of items item tag... Ignore as well
  if(tag == 0xFFFEE000) continue;

  // Even for pixel data, we don't handle very large element lengths
  ...
  // Read contents. Allocate a bit more to make room for casts to int below.
  ...
  layer_ID = gimp_layer_new(image_ID, _("Background"), dicominfo->width, dicominfo->height,
    (dicominfo->samples_per_pixel >= 3 ? GIMP_RGB_IMAGE : GIMP_GRAY_IMAGE), 100, GIMP_NORMAL_MODE);
...
#if GUESS_ENDIAN
  if(bpp == 16) guess_and_set_endian2((guint16*)pix_buf, width*height);
#endif
...
static void dicom_loader(guint8 *pix_buffer, DicomInfo *info, GimpPixelRgn *pixel_rgn)
{
  guchar  *data;
  gint     row_idx;
  ...
  for(row_idx = 0; row_idx < height;)
  {
    ...
    for(i = 0; i < scanlines; i++)
    {
      if(info->bpp == 16)
      {
        ...
        {
          // Shift it by 8 bits, or less in case bits_stored is less than bpp.
      ...
      else if(info->bpp == 8)
      {
        ...
        for (col_idx = 0; col_idx < width * samples_per_pixel; col_idx++)
        {
          // Shift it by 0 bits, or more in case bits_stored is less than bpp.
...
// Guess and set endian. Guesses the endian of a buffer by checking the maximum
// value of the first and the last byte in the words of the buffer. It assumes
// that the least significant byte has a larger maximum than the most significant byte.
static void guess_and_set_endian2(guint16 *buf16, int length)
{
  guint16 *p = buf16;
  gint     max_first  = -1;
  gint     max_second = -1;
  ...
  {
    if(*(guint8*)p > max_first) max_first = *(guint8*)p;
    if(((guint8*)p)[1] > max_second) max_second = ((guint8*)p)[1];
    p++;
  }

  if(((max_second > max_first) && (G_BYTE_ORDER == G_LITTLE_ENDIAN))
    || ((max_second < max_first) && (G_BYTE_ORDER == G_BIG_ENDIAN))) toggle_endian2(buf16, length);
}
...
// save_image() saves an image in the dicom format. The DICOM format requires
// a lot of tags to be set. Some of them have real uses, others must just be
...
static gboolean save_image(const gchar *filename, gint32 image_ID, gint32 drawable_ID, GError **error)
{
  ...
  g_snprintf(today_string, sizeof(today_string), "%04d%02d%02d", date->year, date->month, date->day);
  g_date_free(date);
  ...
  // Print dicom header
  {
    ...
    for(i = 0; i < 0x80; i++) fwrite(&val, 1, 1, DICOM);
  }
  fprintf(DICOM, "DICM");
  ...
  add_tag_pointer(group_stream, group, 0x0001, "OB", (const guint8*)"\0\1", 2);
...
static void add_tag_pointer(GByteArray *group_stream, gint group, gint element,
  const gchar *value_rep, const guint8 *data, gint length)
{
  gboolean is_long;
  guint16  swapped16;
  guint32  swapped32;
...
static void add_tag_string(GByteArray *group_stream, gint group, gint element,
  const gchar *value_rep, const gchar *s)
{
  add_tag_pointer(group_stream, group, element, value_rep, (const guint8*)s, strlen(s));
}
...
static void add_tag_int(GByteArray *group_stream, gint group, gint element, const gchar *value_rep, gint value)
{
  ...
  if(strcmp(value_rep, "US") == 0) len = 2;
  else len = 4;

  add_tag_pointer(group_stream, group, element, value_rep, (const guint8*)&value, len);
}

// Once a group has been built it has to be wrapped with a meta-group tag before
// it is written to the DICOM file. This is done by write_group_to_file.
static gboolean write_group_to_file(FILE *DICOM, gint group, GByteArray *group_stream)
{
  gboolean retval = TRUE;
  guint16  swapped16;
