...
static void query(void)
{
  ...
  gimp_install_procedure(PLUG_IN_PROC, N_("Several simple methods for detecting edges"), help_string,
    "Peter Mattis & (ported to 1.0 by) Eiichi Takamori", "Peter Mattis", "1996", N_("_Edge..."),
    "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS (args), 0, args, NULL);
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  *return_vals  = values;
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    if(!edge_dialog(drawable)) return;
    break;
  ...
  if(gimp_drawable_is_rgb(drawable->drawable_id) || gimp_drawable_is_gray(drawable->drawable_id))
  {
    ...
    // Store data
    if(run_mode == GIMP_RUN_INTERACTIVE) gimp_set_data(PLUG_IN_PROC, &evals, sizeof(EdgeVals));
  }
  else
  {
...
static void edge(GimpDrawable *drawable)
{
  ...
  for(pr = gimp_pixel_rgns_register(2, &src_rgn, &dest_rgn); pr != NULL; pr = gimp_pixel_rgns_process(pr))
  {
    ...
    for(y = dest_rgn.y; y < (dest_rgn.y + dest_rgn.h); y++, srcrow += src_rgn.rowstride, destrow += dest_rgn.rowstride)
    {
      src  = srcrow;
      dest = destrow;

      for(x = dest_rgn.x; x < (dest_rgn.x + dest_rgn.w); x++, src += src_rgn.bpp, dest += dest_rgn.bpp)
      {
        if(dest_rgn.x < x && x < dest_rgn.x + dest_rgn.w - 2 && dest_rgn.y < y && y < dest_rgn.y + dest_rgn.h - 2)
        {
          // 3x3 kernel is inside of the tile -- do fast version
          for(chan = 0; chan < alpha; chan++)
          {
            // get the 3x3 kernel into a guchar array and send it to edge_detect
            guchar kernel[9];
            gint   i,j;
...
#undef  PIX
        ...
        {
          // The kernel is not inside of the tile -- do slow version
        ...
        if(has_alpha) dest[alpha] = src[alpha];
      }
    }
...
static gint sobel(const guchar *data)
{
  ...
  return sqrt(v_grad*v_grad*evals.amount + h_grad*h_grad*evals.amount);
}


// Edge detector via template matting -- Prewitt Compass
static gint prewitt(const guchar *data)
{
  gint k, max;
  gint m[8];
...
static gboolean edge_dialog(GimpDrawable *drawable)
{
  ...
  dialog = gimp_dialog_new(_("Edge Detection"), PLUG_IN_BINARY, NULL, 0, gimp_standard_help_func,
  ...
  g_signal_connect(preview, "invalidated", G_CALLBACK(edge_preview_update), NULL);
  ...
  combo = gimp_int_combo_box_new(_("Sobel"), SOBEL, _("Prewitt compass"), PREWITT,
    _("Gradient"), GRADIENT, _("Roberts"), ROBERTS, _("Differential"), DIFFERENTIAL,
    _("Laplace"), LAPLACE, NULL);

  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), evals.edgemode,
    G_CALLBACK(gimp_int_combo_box_get_active), &evals.edgemode);

  gimp_table_attach_aligned(GTK_TABLE(table), 0, 0, _("_Algorithm:"), 0.0, 0.5, combo, 2, FALSE);
  g_signal_connect_swapped(combo, "changed", G_CALLBACK(gimp_preview_invalidate), preview);

  // Label, scale, entry for evals.amount
  scale_data = gimp_scale_entry_new(GTK_TABLE(table), 0, 1, _("A_mount:"), 100, 0,
    evals.amount, 1.0, 10.0, 0.1, 1.0, 1, FALSE, 1.0, G_MAXFLOAT, NULL, NULL);

  g_signal_connect(scale_data, "value-changed", G_CALLBACK(gimp_double_adjustment_update), &evals.amount);
  ...
  gtk_table_attach(GTK_TABLE(table), hbox, 0, 3, 2, 3, GTK_FILL, GTK_FILL, 0, 0);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update), &use_wrap);
  g_signal_connect_swapped(toggle, "toggled", G_CALLBACK(gimp_preview_invalidate), preview);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update), &use_smear);
  g_signal_connect_swapped(toggle, "toggled", G_CALLBACK(gimp_preview_invalidate), preview);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update), &use_black);
  g_signal_connect_swapped(toggle, "toggled", G_CALLBACK(gimp_preview_invalidate), preview);
...
#undef SRC
      ...
      if(has_alpha) dest[alpha] = src[bytes*(x + width*y) + alpha];
      dest += bytes;
    }
