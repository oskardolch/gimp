...
static void neon(GimpDrawable *drawable, gdouble radius, gdouble amount, GimpPreview *preview);
...
static void find_constants(gdouble n_p[], gdouble n_m[], gdouble d_p[], gdouble d_m[],
  gdouble bd_p[], gdouble bd_m[], gdouble std_dev);
static void transfer_pixels(gdouble *src1, gdouble *src2, guchar *dest, gint bytes, gint width);
static void combine_to_gradient(guchar *dest, guchar *src2, gint bytes, gint width, gdouble amount);
...
static void query(void)
{
  ...
  gimp_install_procedure(PLUG_IN_PROC, N_("Simulate the glowing boundary of a neon light"),
    help_string, "Spencer Kimball", "Bit Specialists, Inc.", "2002", N_("_Neon..."), "RGB*, GRAY*",
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  *return_vals  = values;
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    if(!neon_dialog(drawable)) return;
    break;
  ...
  if(gimp_drawable_is_rgb (drawable->drawable_id) || gimp_drawable_is_gray (drawable->drawable_id))
  {
    ...
    // Store data
    if(run_mode == GIMP_RUN_INTERACTIVE) gimp_set_data(PLUG_IN_PROC, &evals, sizeof(NeonVals));
  }
  else
  {
...
static void neon(GimpDrawable *drawable, gdouble radius, gdouble amount, GimpPreview *preview)
{
  ...
  for(col = 0; col < width; col++)
  {
    ...
    {
      for(row = 0 ; row < height ; row++)
        memcpy(preview_buffer1 + (row*width + col)*bytes, dest + bytes*row, bytes);
    }
    else
    {
      ...
      if((col % 20) == 0) gimp_progress_update((double)progress/(double)max_progress);
    }
  }
  ...
  for(row = 0; row < height; row++)
  {
    ...
    if(preview)
    {
      memcpy(src2, preview_buffer1 + row*width*bytes, width*bytes);
    }
    else
    {
    ...
    if(preview)
    {
      memcpy(preview_buffer2 + row * width * bytes, dest, width * bytes);
    }
    else
    {
      ...
      if((row % 20) == 0) gimp_progress_update((double)progress/(double)max_progress);
    }
  }
...
static gboolean neon_dialog(GimpDrawable *drawable)
{
  ...
  dialog = gimp_dialog_new(_("Neon Detection"), PLUG_IN_BINARY, NULL, 0, gimp_standard_help_func,
  ...
  g_signal_connect(preview, "invalidated", G_CALLBACK(neon_preview_update), NULL);
  ...
  scale_data = gimp_scale_entry_new(GTK_TABLE(table), 0, 0, _("_Radius:"), 100, 8, evals.radius,
    0.0, 64.0, 1, 10, 2, FALSE, 0.0, 8*MAX(drawable->width, drawable->height), NULL, NULL);

  g_signal_connect(scale_data, "value-changed", G_CALLBACK(gimp_double_adjustment_update), &evals.radius);
  ...
  scale_data = gimp_scale_entry_new(GTK_TABLE(table), 0, 1, _("_Amount:"), 100, 8, evals.amount,
    0.0, 1.0, 0.01, 0.1, 2, TRUE, 0, 0, NULL, NULL);
...
static void neon_preview_update(GimpPreview *preview)
{
  neon(GIMP_DRAWABLE_PREVIEW(preview)->drawable, evals.radius, evals.amount, preview);
}
