...
static void query(void)
{
  ...
  gimp_install_procedure(LOAD_PROC, "Loads files in KISS CEL file format",
    "This plug-in loads individual KISS cell files.", "Nick Lamb", "Nick Lamb <njl195@zepler.org.uk>",
    "May 1998", N_("KISS CEL"), NULL, GIMP_PLUGIN, G_N_ELEMENTS(load_args), G_N_ELEMENTS(load_return_vals),
    load_args, load_return_vals);

  gimp_register_magic_load_handler(LOAD_PROC, "cel", "", "0,string,KiSS\\040");

  gimp_install_procedure(SAVE_PROC, "Saves files in KISS CEL file format", "This plug-in saves individual KISS cell files.",
    "Nick Lamb", "Nick Lamb <njl195@zepler.org.uk>", "May 1998", N_("KISS CEL"), "RGB*, INDEXED*", GIMP_PLUGIN,
  ...
  *return_vals  = values;
  ...
  if(strcmp(name, LOAD_PROC) == 0)
  {
    ...
    if(run_mode == GIMP_RUN_NONINTERACTIVE)
    {
      ...
      if(palette_file) data_length = strlen(palette_file) + 1;
      else data_length = 0;
    }
    else if(run_mode == GIMP_RUN_INTERACTIVE)
    {
      ...
      if(!error)
      {
        if(needs_palette) palette_dialog(_("Load KISS Palette"));
  ...
  else if(strcmp(name, SAVE_PROC) == 0)
  {
    ...
    case GIMP_RUN_WITH_LAST_VALS:
      ...
      export = gimp_export_image(&image_ID, &drawable_ID, "CEL",
        (GIMP_EXPORT_CAN_HANDLE_RGB | GIMP_EXPORT_CAN_HANDLE_ALPHA | GIMP_EXPORT_CAN_HANDLE_INDEXED));
...
static gboolean need_palette(const gchar *file, GError **error)
{
  ...
  if(fp == NULL)
  {
    g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
      _("Could not open '%s' for reading: %s"), gimp_filename_to_utf8(file), g_strerror(errno));
    return FALSE;
  }
...
static gint32 load_image(const gchar *file, const gchar *brief, GError **error)
{
  ...
  { // New-style image file, read full header
    ...
    {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("is not a CEL image file"));
      return -1;
    }
  ...
  {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
      _("illegal image dimensions: width: %d, horizontal offset: %d, height: %d, vertical offset: %d"),
  ...
  for(i = 0; i < height && !feof(fp); ++i)
  {
    ...
    case 32:
      ...
      // The CEL file order is BGR so we need to swap B and R to get the Gimp RGB order.
      for(j = 0; j < width; j++)
      {
        guint8 tmp = line[j*4];
        line[j*4] = line[j*4+2];
      ...
      break;
    default:
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("Unsupported bit depth (%d)!"), bpp);
      return -1;
    }
  ...
  if(bpp != 32)
  {
    ...
    else
    {
      ...
      {
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
          _("Could not open '%s' for reading: %s"), gimp_filename_to_utf8(palette_file), g_strerror(errno));
        return -1;
      }
    }
...
static gint load_palette(const gchar *file, FILE *fp, guchar palette[], GError **error)
{
  ...
  if(!strncmp((const gchar*)header, "KiSS", 4))
  {
    ...
    {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
        _("'%s': is not a KCF palette file"), gimp_filename_to_utf8(file));
      return -1;
    }
    ...
    {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
        _("'%s': illegal bpp value in palette: %hhu"), gimp_filename_to_utf8(file), bpp);
      return -1;
    }
    ...
    {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
        _("'%s': illegal number of colors: %u"), gimp_filename_to_utf8(file), colours);
      return -1;
    }
    ...
    case 12:
      for(i = 0; i < colours; ++i)
      {
        ...
        {
          g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
            _("'%s': EOF or error while reading palette data"), gimp_filename_to_utf8(file));
          return -1;
        }
...
static gboolean save_image(const gchar *file, const gchar *brief, gint32 image, gint32 layer, GError **error)
{
  ...
  if(type != GIMP_INDEXEDA_IMAGE) bpp = 32;
  else bpp = 4;
