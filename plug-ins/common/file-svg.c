...
static GdkPixbuf* load_rsvg_pixbuf(const gchar *filename, SvgLoadVals *vals, GError **error);
static gboolean load_rsvg_size(const gchar *filename, SvgLoadVals *vals, GError **error);
static GimpPDBStatusType load_dialog(const gchar *filename, GError **error);
...
static void query(void)
{
  ...
  gimp_install_procedure(LOAD_PROC, "Loads files in the SVG file format",
    "Renders SVG files to raster graphics using librsvg.", "Dom Lachowicz, Sven Neumann",
    "Dom Lachowicz <cinamod@hotmail.com>", SVG_VERSION, N_("SVG image"), NULL, GIMP_PLUGIN,
  ...
  gimp_register_magic_load_handler(LOAD_PROC, "svg", "", "0,string,<?xml,0,string,<svg");

  gimp_install_procedure(LOAD_THUMB_PROC, "Loads a small preview from an SVG image", "",
    "Dom Lachowicz, Sven Neumann", "Dom Lachowicz <cinamod@hotmail.com>", SVG_VERSION, NULL, NULL,
    GIMP_PLUGIN, G_N_ELEMENTS(thumb_args), G_N_ELEMENTS(thumb_return_vals), thumb_args, thumb_return_vals);
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  *return_vals  = values;
  ...
  if(strcmp(name, LOAD_PROC) == 0)
  {
    ...
    // Don't clamp this; insane values are probably not meant to be used as resolution anyway.
    if(load_vals.resolution < GIMP_MIN_RESOLUTION || load_vals.resolution > GIMP_MAX_RESOLUTION)
    {
    ...
    if(status == GIMP_PDB_SUCCESS)
    {
      ...
      if(image_ID != -1)
      {
        if(load_vals.import)
        {
          ...
          gimp_vectors_import_from_file(image_ID, filename, load_vals.merge, TRUE,
            &num_vectors, &vectors);
          if(num_vectors > 0) g_free (vectors);
        }
...
static gint32 load_image(const gchar *filename, GError **load_error)
{
  gint32        image;
  gint32        layer;
  GdkPixbuf    *pixbuf;
  ...
  if(!pixbuf)
  {
    ...
    g_set_error(load_error, error ? error->domain : 0, error ? error->code : 0,
      _("Could not open '%s' for reading: %s"), gimp_filename_to_utf8(filename),
  ...
  gimp_image_set_resolution(image, load_vals.resolution, load_vals.resolution);
...
static void load_set_size_callback(gint *width, gint *height, gpointer data)
{
  ...
  if(!vals->width || !vals->height) return;

  // either both arguments negative or none
  if((vals->width * vals->height) < 0) return;
...
static RsvgHandle* load_rsvg_handle_new(gdouble xres, gdouble yres)
{
...
static GdkPixbuf* load_rsvg_pixbuf(const gchar *filename, SvgLoadVals *vals, GError **error)
{
  ...
  while(success && status != G_IO_STATUS_EOF)
  {
    ...
    case G_IO_STATUS_NORMAL:
      success = rsvg_handle_write(handle, (const guchar*)buf, len, error);
      break;
  ...
  if(success) pixbuf = rsvg_handle_get_pixbuf(handle);
...
static gboolean load_rsvg_size(const gchar *filename, SvgLoadVals *vals, GError **error)
{
  ...
  if(size_label)
  {
    if(done)
    {
      gchar *text = g_strdup_printf(_("%d × %d"), vals->width, vals->height);
      gtk_label_set_text(GTK_LABEL(size_label), text);
      g_free(text);
    }
    else
    {
      gtk_label_set_text(GTK_LABEL(size_label), _("SVG file does not\nspecify a size!"));
    }
  }
...
static void load_dialog_set_ratio(gdouble x, gdouble y);

static void load_dialog_size_callback(GtkWidget *widget, gpointer data)
{
...
static void load_dialog_ratio_callback(GtkAdjustment *adj, gpointer data)
{
...
static void load_dialog_resolution_callback(GimpSizeEntry *res, const gchar *filename)
{
  SvgLoadVals vals = { 0.0, 0, 0 };

  load_vals.resolution = vals.resolution = gimp_size_entry_get_refval(res, 0);

  if(!load_rsvg_size(filename, &vals, NULL)) return;
...
static GimpPDBStatusType load_dialog(const gchar *filename, GError **load_error)
{
  GtkWidget   *dialog;
  GtkWidget   *frame;
  GtkWidget   *hbox;
  ...
  dialog = gimp_dialog_new(_("Render Scalable Vector Graphics"), PLUG_IN_BINARY, NULL, 0,
    gimp_standard_help_func, LOAD_PROC, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
  ...
  size = GIMP_SIZE_ENTRY(gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a",
    TRUE, FALSE, FALSE, 10, GIMP_SIZE_ENTRY_UPDATE_SIZE));
  ...
  gimp_size_entry_set_refval_boundaries(size, 0, GIMP_MIN_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE);
  gimp_size_entry_set_refval_boundaries (size, 1, GIMP_MIN_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE);
  ...
  g_signal_connect(size, "value-changed", G_CALLBACK(load_dialog_size_callback), NULL);
  ...
  spinbutton = gimp_spin_button_new(&xadj, ratio_x,
  ...
  g_signal_connect(xadj, "value-changed", G_CALLBACK(load_dialog_ratio_callback), NULL);
  ...
  spinbutton = gimp_spin_button_new(&yadj, ratio_y,
  ...
  g_signal_connect(yadj, "value-changed", G_CALLBACK(load_dialog_ratio_callback), NULL);
  ...
  gimp_help_set_help_data(GIMP_CHAIN_BUTTON(constrain)->button, _("Constrain aspect ratio"), NULL);
  ...
  res = gimp_size_entry_new(1, GIMP_UNIT_INCH, _("pixels/%a"), FALSE, FALSE, FALSE, 10,
  ...
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(res), 0, 5.0, GIMP_MAX_RESOLUTION);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(res), 0, load_vals.resolution);

  g_signal_connect(res, "value-changed", G_CALLBACK(load_dialog_resolution_callback), (gpointer)filename);
  ...
  gimp_help_set_help_data(toggle,
    _("Import path elements of the SVG so they can be used with the GIMP path tool"), NULL);

  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), load_vals.import);
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update), &load_vals.import);

  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_sensitive_update), NULL);
  ...
  g_signal_connect(toggle2, "toggled", G_CALLBACK(gimp_toggle_button_update), &load_vals.merge);
