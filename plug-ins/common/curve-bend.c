...
static void bender_response(GtkWidget*, gint, BenderDialog*);
...
static void bender_load_callback(GtkWidget*, BenderDialog*);
static void bender_save_callback(GtkWidget*, BenderDialog*);
...
static void p_init_gdrw(t_GDRW *gdrw, GimpDrawable *drawable, int dirty, int shadow);
...
static void p_put_mix_pixel(t_GDRW *gdrw, gint32 x, gint32 y, guchar *color,
  gint32 nb_curvy, gint32 nb2_curvy, gint32 curvy);
...
static void p_copy_yval(BenderDialog *cd, int outline, int argc, guint8 *int8array);
...
static gint p_pdb_procedure_available(const gchar *proc_name)
{
  ...
  if(gimp_procedural_db_proc_info(proc_name, &l_proc_blurb, &l_proc_help, &l_proc_author,
    &l_proc_copyright, &l_proc_date, &l_proc_type, &l_nparams, &l_nreturn_vals, &l_params, &l_return_vals))
  {
...
static gint p_gimp_rotate(gint32 image_id, gint32 drawable_id, gint32 interpolation, gdouble angle_deg)
{
  gdouble l_angle_rad;
  gint    l_rc;
  ...
  if(l_angle_step != 0)
  {
    ...
    if(l_nparams == 5)
    {
      ...
      return_vals = gimp_run_procedure(l_rotate_proc, &nreturn_vals, GIMP_PDB_INT32, GIMP_RUN_NONINTERACTIVE,
        GIMP_PDB_IMAGE, image_id, GIMP_PDB_DRAWABLE, drawable_id, GIMP_PDB_INT32, l_angle_step, GIMP_PDB_INT32,
        FALSE, // dont rotate the whole image
        GIMP_PDB_END);
  ...
  l_rc = gimp_drawable_transform_rotate_default(drawable_id, l_angle_rad, TRUE, 0, 0, interpolation, FALSE);
...
static gint32 p_if_selection_float_it(gint32 image_id, gint32 layer_id)
{
...
static void query(void)
{
  ...
  gimp_install_procedure(PLUG_IN_PROC,
    ...
    "same position",
    PLUG_IN_AUTHOR, PLUG_IN_COPYRIGHT, PLUG_IN_VERSION, N_("_Curve Bend..."), PLUG_IN_IMAGE_TYPES,
    GIMP_PLUGIN, G_N_ELEMENTS(args), G_N_ELEMENTS(return_vals), args, return_vals);
  ...
  gimp_install_procedure(PLUG_IN_ITER_NAME,
    ...
    "plug_in_curve_bend",
    "",
    PLUG_IN_AUTHOR, PLUG_IN_COPYRIGHT, PLUG_IN_VERSION,
    NULL, // do not appear in menus
    NULL, GIMP_PLUGIN, G_N_ELEMENTS(args_iter), 0, args_iter, NULL);
}
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  *return_vals  = values;
  ...
  if(strcmp(name, PLUG_IN_ITER_NAME) == 0)
  {
    ...
    // "plug_in_gap_layers_run_animfilter" (always run noninteractive)
  ...
  {
    g_set_error(&error, 0, 0, "%s", _("Can operate on layers only (but was called on channel or mask)."));
  ...
  {
    g_set_error(&error, 0, 0, "%s", _("Cannot operate on layers with masks."));
    status = GIMP_PDB_EXECUTION_ERROR;
  }
  ...
  if(l_active_drawable_id < 0)
  {
    ...
    g_set_error(&error, 0, 0, "%s", _("Cannot operate on empty selections."));
    status = GIMP_PDB_EXECUTION_ERROR;
  }
  else
  {
  ...
  if(status == GIMP_PDB_SUCCESS)
  {
    ...
    case GIMP_RUN_NONINTERACTIVE:
      ...
      if(nparams >= 20)
      {
        ...
        p_copy_points(cd, OUTLINE_UPPER, 0, param[8].data.d_int32, param[9].data.d_floatarray);
        p_copy_points(cd, OUTLINE_UPPER, 1, param[10].data.d_int32, param[11].data.d_floatarray);
        p_copy_points(cd, OUTLINE_LOWER, 0, param[12].data.d_int32, param[13].data.d_floatarray);
        p_copy_points(cd, OUTLINE_LOWER, 1, param[14].data.d_int32, param[15].data.d_floatarray);

        p_copy_yval(cd, OUTLINE_UPPER, param[16].data.d_int32, param[17].data.d_int8array);
        p_copy_yval(cd, OUTLINE_LOWER, param[18].data.d_int32, param[19].data.d_int8array);
      }
      else
      {
        status = GIMP_PDB_CALLING_ERROR;
...
static int p_save_pointfile(BenderDialog *cd, const gchar *filename)
{
  ...
  for(j = 0; j < 256; j++)
  {
    fprintf(l_fp, "%s %3d  %3d\n", KEY_VAL_Y, (int)cd->curve[OUTLINE_UPPER][j], (int)cd->curve[OUTLINE_LOWER][j]);
  }
...
static int p_load_pointfile(BenderDialog *cd, const gchar *filename)
{
...
static BenderDialog* bender_new_dialog(GimpDrawable *drawable)
{
  ...
  cd->shell = gimp_dialog_new(_("Curve Bend"), PLUG_IN_BINARY, NULL, 0, gimp_standard_help_func,
    PLUG_IN_PROC, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);

  gtk_dialog_set_alternative_button_order(GTK_DIALOG (cd->shell), GTK_RESPONSE_OK,
    GTK_RESPONSE_CANCEL, -1);

  gimp_window_set_transient(GTK_WINDOW(cd->shell));

  g_signal_connect(cd->shell, "response", G_CALLBACK(bender_response), cd);
  ...
  gtk_widget_set_size_request(cd->pv_widget, PREVIEW_SIZE_X, PREVIEW_SIZE_Y);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(bender_preview_update_once), cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_preview_update), cd);
  ...
  spinbutton = gimp_spin_button_new(&data, 0, 0.0, 360.0, 1, 45, 90, 0.5, 1);
  ...
  g_signal_connect(cd->rotate_data, "value-changed", G_CALLBACK(bender_rotate_adj_callback), cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_smoothing_callback), cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_antialias_callback), cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_work_on_copy_callback), cd);
  ...
  gtk_widget_set_size_request(cd->graph, GRAPH_WIDTH + RADIUS*2, GRAPH_HEIGHT + RADIUS*2);
  gtk_widget_set_events(cd->graph, GRAPH_MASK);
  gtk_container_add(GTK_CONTAINER(abox), cd->graph);
  gtk_widget_show(cd->graph);

  g_signal_connect(cd->graph, "event", G_CALLBACK(bender_graph_events), cd);

  hbox = gtk_hbox_new(FALSE, 12);
  gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
  gtk_widget_show(hbox);

  frame = gimp_int_radio_group_new(TRUE, _("Curve for Border"), G_CALLBACK(bender_border_callback),
    &cd->outline, cd->outline, _("_Upper"), OUTLINE_UPPER, &upper, _("_Lower"), OUTLINE_LOWER, &lower, NULL);
  ...
  frame = gimp_int_radio_group_new(TRUE, _("Curve Type"), G_CALLBACK(bender_type_callback),
    &cd->curve_type, cd->curve_type, _("Smoot_h"), SMOOTH, &smooth, _("_Free"), GFREE, &freew, NULL);
  ...
  gimp_help_set_help_data(button, _("Copy the active curve to the other border"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(bender_copy_callback), cd);
  ...
  gimp_help_set_help_data(button, _("Mirror the active curve to the other border"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(bender_copy_inv_callback), cd);
  ...
  gimp_help_set_help_data(button, _("Swap the two curves"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK (bender_swap_callback), cd);
  ...
  gimp_help_set_help_data(button, _("Reset the active curve"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(bender_reset_callback), cd);
  ...
  gimp_help_set_help_data(button, _("Load the curves from a file"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(bender_load_callback), cd);
  ...
  gimp_help_set_help_data(button, _("Save the curves to a file"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(bender_save_callback), cd);
...
static void bender_update(BenderDialog *cd, int update)
{
  ...
  if(update & UP_PREVIEW)
  {
    ...
    cd->preview_layer_id2 = p_main_bend(cd, gimp_drawable_get(cd->preview_layer_id1),
      TRUE // work_on_copy
      );
    p_render_preview(cd, cd->preview_layer_id2);

    if(update & UP_DRAW) gtk_widget_queue_draw(cd->pv_widget);
  ...
  if((update & UP_GRAPH) && (update & UP_DRAW) && cd->pixmap != NULL)
  {
    ...
    for(i = 0; i < 5; i++)
    {
      gdk_draw_line(cd->pixmap, graph_style->dark_gc[GTK_STATE_NORMAL], RADIUS, i*(GRAPH_HEIGHT/4) + RADIUS,
        GRAPH_WIDTH + RADIUS, i*(GRAPH_HEIGHT/4) + RADIUS);
      gdk_draw_line(cd->pixmap, graph_style->dark_gc[GTK_STATE_NORMAL], i*(GRAPH_WIDTH/4) + RADIUS, RADIUS,
    ...
    if(cd->curve_type == SMOOTH)
    {
      for(i = 0; i < 17; i++)
      {
        if(cd->points[cd->outline][i][0] != -1)
          gdk_draw_arc(cd->pixmap, graph_style->black_gc, TRUE, (cd->points[cd->outline][i][0]*255.0),
            255 - (cd->points[cd->outline][i][1]*255.0), RADIUS*2, RADIUS*2, 0, 23040);
      }
    }
...
static void bender_plot_curve(BenderDialog *cd, int p1, int p2, int p3, int p4,
  gint32 xmax, gint32 ymax, gint fix255)
{
  CRMatrix geometry;
  CRMatrix tmp1, tmp2;
  CRMatrix deltas;
...
static void bender_calculate_curve(BenderDialog *cd, gint32 xmax, gint32 ymax, gint fix255)
{
  int i;
  int points[17];
  int num_pts;
  ...
  case SMOOTH:
    ...
    num_pts = 0;
    for(i = 0; i < 17; i++)
      if(cd->points[cd->outline][i][0] != -1) points[num_pts++] = i;
...
static void bender_rotate_adj_callback(GtkAdjustment *adjustment, gpointer client_data)
{
...
static void bender_border_callback(GtkWidget *widget, gpointer data)
{
  BenderDialog *cd;
...
static void bender_type_callback(GtkWidget *widget, gpointer data)
{
  BenderDialog *cd;
...
static void bender_reset_callback(GtkWidget *widget, gpointer client_data)
{
...
static void bender_copy_callback(GtkWidget *widget, gpointer client_data)
{
  BenderDialog *cd = (BenderDialog*)client_data;
  int i;
  int other;
...
static void bender_copy_inv_callback(GtkWidget *widget, gpointer client_data)
{
  BenderDialog *cd = (BenderDialog*)client_data;
  int i;
  int other;
...
static void bender_swap_callback(GtkWidget *widget, gpointer client_data)
{
...
static void bender_response(GtkWidget *widget, gint response_id, BenderDialog *cd)
{
...
static void bender_preview_update(GtkWidget *widget, gpointer data)
{
...
static void bender_preview_update_once(GtkWidget *widget, gpointer data)
{
  BenderDialog *cd = (BenderDialog*)data;
  bender_update(cd, UP_PREVIEW | UP_DRAW);
}

static void p_points_save_to_file_response(GtkWidget *dialog, gint response_id, BenderDialog *cd)
{
  if(response_id == GTK_RESPONSE_OK)
  {
    gchar *filename;
...
static void p_points_load_from_file_response(GtkWidget *dialog, gint response_id, BenderDialog *cd)
{
  if(response_id == GTK_RESPONSE_OK)
  {
    gchar *filename;
...
static void bender_load_callback(GtkWidget *w, BenderDialog *cd)
{
  if(!cd->filechooser)
  {
    cd->filechooser = gtk_file_chooser_dialog_new(_("Load Curve Points from File"),
      GTK_WINDOW(gtk_widget_get_toplevel(w)), GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL,
      GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_OK, NULL);

    gtk_dialog_set_alternative_button_order(GTK_DIALOG(cd->filechooser), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

    gtk_dialog_set_default_response(GTK_DIALOG(cd->filechooser), GTK_RESPONSE_OK);

    g_signal_connect(cd->filechooser, "response", G_CALLBACK(p_points_load_from_file_response), cd);
    g_signal_connect(cd->filechooser, "destroy", G_CALLBACK(gtk_widget_destroyed), &cd->filechooser);
  }

  gtk_window_present(GTK_WINDOW(cd->filechooser));
}

static void bender_save_callback(GtkWidget *w, BenderDialog *cd)
{
  if(!cd->filechooser)
  {
    cd->filechooser = gtk_file_chooser_dialog_new(_("Save Curve Points to File"),
      GTK_WINDOW(gtk_widget_get_toplevel(w)), GTK_FILE_CHOOSER_ACTION_SAVE,
      GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_OK, NULL);

    g_signal_connect(cd->filechooser, "response", G_CALLBACK(p_points_save_to_file_response), cd);
    g_signal_connect(cd->filechooser, "destroy", G_CALLBACK(gtk_widget_destroyed), &cd->filechooser);

    gtk_file_chooser_set_current_name(GTK_FILE_CHOOSER(cd->filechooser), "curve_bend.points");
  }

  gtk_window_present(GTK_WINDOW (cd->filechooser));
}

static void bender_smoothing_callback(GtkWidget *w, gpointer data)
{
  BenderDialog *cd = (BenderDialog*)data;
...
static void bender_antialias_callback(GtkWidget *w, gpointer data)
{
  BenderDialog *cd = (BenderDialog*)data;
...
static void bender_work_on_copy_callback(GtkWidget *w, gpointer data)
{
  BenderDialog *cd = (BenderDialog*)data;
...
static gboolean bender_graph_events(GtkWidget *widget, GdkEvent *event, BenderDialog *cd)
{
  ...
  case GDK_EXPOSE:
    if(cd->pixmap == NULL)
      cd->pixmap = gdk_pixmap_new(cd->graph->window, GRAPH_WIDTH + RADIUS*2, GRAPH_HEIGHT + RADIUS*2, -1);
...
static void bender_CR_compose(CRMatrix a, CRMatrix b, CRMatrix ab)
{
  gint i, j;
...
static void p_render_preview(BenderDialog *cd, gint32 layer_id)
{
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(cd->pv_widget), 0, 0, PREVIEW_SIZE_X, PREVIEW_SIZE_Y,
    GIMP_RGBA_IMAGE, l_buf, PREVIEW_BPP*PREVIEW_SIZE_X);
  g_free(l_buf);
...
static void p_stretch_curves(BenderDialog *cd, gint32 xmax, gint32 ymax)
{
  ...
  for(l_outline = 0; l_outline < 2; l_outline++)
  {
    for(l_x1 = 0; l_x1 <= xmax; l_x1++)
    {
      ...
      //{
      //  int l_debugY = ROUND((cd->curve[l_outline][l_x2]*ymax)/255);
      //}
    }
  }
}

static void bender_init_min_max(BenderDialog *cd, gint32 xmax)
{
  int i, j;
...
static gint32 p_curve_get_dy(BenderDialog *cd, gint32 x, gint32 drawable_width,
...
static gint32 p_upper_curve_extend(BenderDialog *cd, gint32 drawable_width, gint32 drawable_height)
{
   gint32 l_y1, l_y2;
...
static gint32 p_lower_curve_extend(BenderDialog *cd, gint32 drawable_width, gint32 drawable_height)
{
   gint32 l_y1, l_y2;
...
static void p_clear_drawable(GimpDrawable *drawable)
{
  ...
  for(pr = gimp_pixel_rgns_register(1, &pixel_rgn); pr != NULL; pr = gimp_pixel_rgns_process(pr))
  {
...
static gint32 p_create_pv_image(GimpDrawable *src_drawable, gint32 *layer_id)
{
  ...
  *layer_id = gimp_layer_new(l_new_image_id, "preview_original",
    l_new_width, l_new_height, l_type,
    100.0, // opacity
    0); // mode NORMAL
...
static GimpDrawable* p_add_layer(gint width, gint height, GimpDrawable *src_drawable)
{
  GimpImageType l_type;
...
static void p_bender_calculate_iter_curve(BenderDialog *cd, gint32 xmax, gint32 ymax)
{
  ...
  else
  {
    ...
    {
      p_delta_gint32(&cd->curve_ptr[OUTLINE_UPPER][l_x], cd_from->curve_ptr[OUTLINE_UPPER][l_x],
        cd_to->curve_ptr[OUTLINE_UPPER][l_x], cd->bval_curr->total_steps, cd->bval_curr->current_step);

      p_delta_gint32(&cd->curve_ptr[OUTLINE_LOWER][l_x], cd_from->curve_ptr[OUTLINE_LOWER][l_x],
        cd_to->curve_ptr[OUTLINE_LOWER][l_x], cd->bval_curr->total_steps, cd->bval_curr->current_step);
    }
...
static void p_vertical_bend(BenderDialog *cd, t_GDRW *src_gdrw, t_GDRW *dst_gdrw)
{
  gint32 l_row, l_col;
    ...
    for(l_col = l_first_col; l_col <= l_last_col; l_col++)
    {
      ...
      for(; l_x < l_x2; l_x++)
      {
        ...
        for(; l_y < l_y2; l_y++)
        {
          ...
          l_curvy = p_curve_get_dy(cd, l_x, (gint32)src_gdrw->drawable->width,
          ...
          if(cd->smoothing && (l_x > 0))
          {
            ...
            {
              l_nb2_curvy = p_curve_get_dy(cd, l_x -2, (gint32)src_gdrw->drawable->width,
                (gint32)src_gdrw->drawable->height, (gdouble)l_y);
            }
            else
            {
          ...
          if(cd->antialias)
          {
            ...
            if(l_y == src_gdrw->y1) // Upper outline
            {
              ...
              memcpy(first_arr[l_x].color, color, dst_gdrw->drawable->bpp);

              if(l_x > 0)
              {
                memcpy(mixcolor, first_arr[l_x-1].color, dst_gdrw->drawable->bpp);

                l_diff = abs(first_arr[l_x - 1].y - l_curvy) + 1;
                l_miny = MIN(first_arr[l_x - 1].y, l_curvy) - 1;
                l_maxy = MAX(first_arr[l_x - 1].y, l_curvy) + 1;

                l_othery = (src_gdrw->y2 - 1) + l_topshift +
                  p_curve_get_dy(cd, l_x, (gint32)src_gdrw->drawable->width, (gint32)src_gdrw->drawable->height,
            ...
            if(l_y == src_gdrw->y2 - 1) // Lower outline
            {
              if(l_x > 0)
              {
                memcpy(mixcolor, last_arr[l_x-1].color, dst_gdrw->drawable->bpp);
              ...
              l_othery = (src_gdrw->y1) + l_topshift +
                p_curve_get_dy(cd, l_x, (gint32)src_gdrw->drawable->width, (gint32)src_gdrw->drawable->height,
                (gdouble)(src_gdrw->y1));
            }
          ...
          for(l_dy = 1; l_dy <= l_diff; l_dy++)
          {
            ...
            if(cd->smoothing)
            {
              ...
              if(mixcolor[3])
              {
                mixcolor[0] = (alpha1*l_mixmask*last_arr[l_x].color[0] + alpha2*(255 - l_mixmask)*color[0])/alpha;
                mixcolor[1] = (alpha1*l_mixmask*last_arr[l_x].color[1] + alpha2*(255 - l_mixmask)*color[1])/alpha;
                mixcolor[2] = (alpha1*l_mixmask*last_arr[l_x].color[2] + alpha2*(255 - l_mixmask)*color[2])/alpha;
            ...
            else
            {
              ...
              {
                memcpy(mixcolor, last_arr[l_x].color, dst_gdrw->drawable->bpp);
              }
            }

            if(cd->smoothing)
            {
              p_put_mix_pixel(dst_gdrw, l_x, l_desty + (l_dy*l_sign), mixcolor, l_nb_curvy, l_nb2_curvy, l_curvy);
            }
            else
            {
              p_put_pixel(dst_gdrw, l_x, l_desty + (l_dy*l_sign), mixcolor);
            }
          }
...
static gint32 p_main_bend(BenderDialog *cd, GimpDrawable *original_drawable, gint work_on_copy)
{
  t_GDRW l_src_gdrw;
  t_GDRW l_dst_gdrw;
  GimpDrawable *dst_drawable;
  ...
  // set layer invisible and dummyname and add at top of the image while working
  // (for the case of undo GIMP must know, that the layer was part of the image)
  ...
  else
  {
    ...
    gimp_layer_resize(original_drawable->drawable_id, src_drawable->width,
      l_dst_height, l_offset_x, l_offset_y);
