...
static gint32 load_image(const gchar *filename, gboolean thumbnail, GError **error);
...
static void query(void)
{
  ...
  gimp_install_procedure(LOAD_PROC, "Loads files of Compuserve GIF file format", "FIXME: write help for gif_load",
    "Spencer Kimball, Peter Mattis, Adam Moss, David Koblas", "Spencer Kimball, Peter Mattis, Adam Moss, David Koblas",
    "1995-2006", N_("GIF image"), NULL, GIMP_PLUGIN, G_N_ELEMENTS(load_args), G_N_ELEMENTS(load_return_vals),
    load_args, load_return_vals);

  gimp_register_file_handler_mime(LOAD_PROC, "image/gif");
  gimp_register_magic_load_handler(LOAD_PROC, "gif", "", "0,string,GIF8");

  gimp_install_procedure(LOAD_THUMB_PROC, "Loads only the first frame of a GIF image, to be used as a thumbnail",
    "", "Sven Neumann", "Sven Neumann", "2006", NULL, NULL, GIMP_PLUGIN, G_N_ELEMENTS(thumb_args),
    G_N_ELEMENTS(load_return_vals), thumb_args, load_return_vals);
...
static gboolean ReadColorMap(FILE *fd, gint number, CMap buffer, gint *format);
static gint DoExtension(FILE *fd, gint label);
static gint GetDataBlock(FILE *fd, guchar *buf);
static gint GetCode(FILE *fd, gint code_size, gboolean flag);
static gint LZWReadByte(FILE *fd, gint just_reset_LZW, gint input_code_size);
static gint32 ReadImage(FILE *fd, const gchar *filename, gint len, gint height, CMap cmap,
  gint ncols, gint format, gint interlace, gint number, guint leftpos, guint toppos,
  guint screenwidth, guint screenheight);
...
static gint32 load_image(const gchar *filename, gboolean thumbnail, GError **error)
{
  ...
  {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "%s", _("This is not a GIF file"));
    return -1;
  }
  ...
  while(TRUE)
  {
    ...
    if(!useGlobalColormap)
    {
      ...
      image_ID = ReadImage(fd, filename, LM_to_uint(buf[4], buf[5]), LM_to_uint(buf[6], buf[7]),
        localColorMap, bitPixel, grayScale, BitSet(buf[8], INTERLACE), imageCount,
        (guint)LM_to_uint(buf[0], buf[1]), (guint)LM_to_uint(buf[2], buf[3]), GifScreen.Width, GifScreen.Height);
    }
    else
    {
      image_ID = ReadImage(fd, filename, LM_to_uint(buf[4], buf[5]), LM_to_uint(buf[6], buf[7]),
        GifScreen.ColorMap, GifScreen.BitPixel, GifScreen.GrayScale, BitSet(buf[8], INTERLACE), imageCount,
        (guint)LM_to_uint(buf[0], buf[1]), (guint)LM_to_uint (buf[2], buf[3]), GifScreen.Width, GifScreen.Height);
    }
...
static gint DoExtension(FILE *fd, gint label)
{
  ...
  case 0xfe: // Comment Extension
    ...
    while(GetDataBlock(fd, (guchar*)buf) > 0)
    {
      ...
      if(!g_utf8_validate(comment, -1, NULL)) continue;
      if(comment_parasite) gimp_parasite_free(comment_parasite);
      comment_parasite = gimp_parasite_new("gimp-comment", GIMP_PARASITE_PERSISTENT, strlen(comment) + 1, comment);
    }
    return TRUE;
...
static gint GetCode(FILE *fd, gint code_size, gboolean flag)
{
  ...
  while((curbit + code_size) >= lastbit)
  {
    if(done)
    {
      if(curbit >= lastbit) g_message("Ran off the end of my bits");
      return -1;
    }
...
#define STACK_SIZE ((1 << (MAX_LZW_BITS)) * 2)
  ...
  while((code = GetCode(fd, code_size, FALSE)) >= 0)
  {
    ...
    if(code == max_code)
    {
      if(sp < &(stack[STACK_SIZE])) *sp++ = firstcode;
      code = oldcode;
    }
    ...
    if(sp < &(stack[STACK_SIZE])) *sp++ = firstcode = table[1][code];
...
static gint32 ReadImage(FILE *fd, const gchar *filename, gint len, gint height,
  CMap cmap, gint ncols, gint format, gint interlace, gint number, guint leftpos,
  guint toppos, guint screenwidth, guint screenheight)
{
  ...
  if(frame_number == 1)
  {
    ...
    if(screenwidth == 0) screenwidth = len;
    ...
    if(Gif89.delayTime < 0) framename = g_strdup(_("Background"));
    else
    ...
    {
      layer_ID = gimp_layer_new(image_ID, framename, len, height,
  ...
  {
    gimp_progress_set_text_printf(_("Opening '%s' (frame %d)"), gimp_filename_to_utf8(filename), frame_number);
    gimp_progress_pulse();
    ...
    if(!promote_to_rgb)
    {
      for(i = 0; i < ncols; i++)
      {
        if((used_cmap[0][i] != cmap[0][i]) || (used_cmap[1][i] != cmap[1][i]) || (used_cmap[2][i] != cmap[2][i]))
        {
    ...
    case 0x02:
      ...
      g_free (framename_ptr);
      break;
    case 0x03: // Rarely-used, and unhandled by many loaders/players
               // (including GIMP: we treat as 'combine' mode).
      framename_ptr = framename;
      framename = g_strconcat(framename, " (combine) (!)", NULL);
      g_free (framename_ptr);
      break;
    ...
    layer_ID = gimp_layer_new(image_ID, framename, len, height,
      promote_to_rgb ? GIMP_RGBA_IMAGE : GIMP_INDEXEDA_IMAGE, 100, GIMP_NORMAL_MODE);
    alpha_frame = TRUE;
    g_free(framename);
  }
  ...
  {
    g_message("'%s' has a larger image size than GIMP can handle.", gimp_filename_to_utf8 (filename));
    return -1;
  }
  ...
  while((v = LZWReadByte (fd, FALSE, c)) >= 0)
  {
    ...
    {
      if((guchar)v > highest_used_index) highest_used_index = (guchar)v;
