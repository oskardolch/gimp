...
static void dialog_new_variations(GtkWidget *widget, gpointer data);
static void dialog_update_previews(GtkWidget *widget, gpointer data);
static void dialog_select_preview(GtkWidget *widget, ExpInfo *n_info);
...
static void modify_info(ExpInfo *o_info, ExpInfo *n_info)
{
  gint k, n;
...
static void check_last_modified(ExpInfo *info, gint p, gint n)
{
  p--;
  while((p >= 0) && (info->dest[p] != n)) p--;
  if(p < 0) used_reg_flag[n] = 1;
  else
  {
    used_trans_flag[p] = 1;
...
static void optimize(ExpInfo *info)
{
  ...
  {
    used_trans_flag[i] = 0;
    if(i < NUM_REGISTERS) used_reg_flag[i] = 0;
    // double-arg fix:
...
static void qbist(ExpInfo *info, guchar *buffer, gint xp, gint yp, gint num,
  gint width, gint height, gint bpp, gint oversampling)
{
  gint gx;
  ...
  for(gx = 0; gx < num; gx++)
  {
    ...
    for(i = 0; i < bpp; i++)
    {
      if(i < 3)
      {
        buffer[i] = (guchar)(((gfloat)accum[i]/(gfloat)(oversampling*oversampling)) + 0.5);
      }
      else buffer[i] = 255;
    }
...
static void query(void)
{
  ...
  gimp_install_procedure(PLUG_IN_PROC, N_("Generate a huge variety of abstract patterns"),
    ...
    "genetic formula.",
    "Jörn Loviscach, Jens Ch. Restemeier", "Jörn Loviscach, Jens Ch. Restemeier", PLUG_IN_VERSION,
    N_("_Qbist..."), "RGB*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);
...
static void run(const gchar *name, gint nparams, const GimpParam *param, gint *nreturn_vals, GimpParam **return_vals)
{
  ...
  gint img_height, img_width; // img_has_alpha, img_bpp;
  ...
  *return_vals  = values;
  ...
  if(!gimp_drawable_is_rgb(drawable->drawable_id)) status = GIMP_PDB_CALLING_ERROR;
  ...
  if(status == GIMP_PDB_SUCCESS)
  {
    ...
    if(status == GIMP_PDB_SUCCESS)
    {
      ...
      for(pr = gimp_pixel_rgns_register(1, &imagePR); pr != NULL; pr = gimp_pixel_rgns_process(pr))
      {
        ...
        {
          qbist(&qbist_info.info, imagePR.data + row*imagePR.rowstride, imagePR.x, imagePR.y + row,
            imagePR.w, sel_x2 - sel_x1, sel_y2 - sel_y1, imagePR.bpp, qbist_info.oversampling);
        }
...
static void dialog_update_previews(GtkWidget *widget, gpointer data)
{
  ...
  for(j = 0; j < 9; j++)
  {
    ...
    {
      qbist(&info[(j + 5) % 9], buf + i*PREVIEW_SIZE*3, 0, i,
        PREVIEW_SIZE, PREVIEW_SIZE, PREVIEW_SIZE, 3, 1);
    }
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview[j]), 0, 0, PREVIEW_SIZE, PREVIEW_SIZE,
      GIMP_RGB_IMAGE, buf, PREVIEW_SIZE*3);
  }
}
...
// File I/O stuff
static guint16 get_be16(guint8 *buf)
{
  return (guint16)(buf[0] << 8 | buf[1]);
}

static void set_be16(guint8 *buf, guint16 val)
{
  buf[0] = val >> 8;
  buf[1] = val & 0xFF;
}

static gboolean load_data(gchar *name)
{
  gint    i;
  FILE   *f;
  guint8  buf[288];
...
static gboolean save_data(gchar *name)
{
  gint    i = 0;
  FILE   *f;
  guint8  buf[288];
...
static void dialog_undo(GtkWidget *widget, gpointer data)
{
  ExpInfo temp_info[9];
...
static void dialog_load(GtkWidget *widget, gpointer data)
{
  GtkWidget *parent;
  GtkWidget *dialog;
  ...
  dialog = gtk_file_chooser_dialog_new(_("Load QBE File"), GTK_WINDOW(parent),
...
static void dialog_save(GtkWidget *widget, gpointer data)
{
  GtkWidget *parent;
  GtkWidget *dialog;

  parent = gtk_widget_get_toplevel(widget);
  dialog = gtk_file_chooser_dialog_new(_("Save as QBE File"), GTK_WINDOW(parent),
...
static void dialog_toggle_antialaising(GtkWidget *widget, gpointer data)
{
...
static gboolean dialog_run(void)
{
  ...
  dialog = gimp_dialog_new(_("G-Qbist"), PLUG_IN_BINARY, NULL, 0,
    gimp_standard_help_func, PLUG_IN_PROC, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
  ...
  for(i = 0; i < 9; i++)
  {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(dialog_select_preview), (gpointer) & (info[(i + 5) % 9]));
  ...
  gtk_widget_show(button);
  g_signal_connect(button, "toggled", G_CALLBACK(dialog_toggle_antialaising), NULL);
  ...
  gtk_widget_show(button);
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_undo), NULL);

  button = gtk_button_new_from_stock(GTK_STOCK_OPEN);
  gtk_container_add(GTK_CONTAINER(bbox), button);
  gtk_widget_show(button);
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_load), NULL);

  button = gtk_button_new_from_stock(GTK_STOCK_SAVE);
  gtk_container_add(GTK_CONTAINER(bbox), button);
  gtk_widget_show(button);
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_save), NULL);
