...
static GTokenType ifsvals_parse_color(GScanner *scanner, GimpRGB *result)
{
  GTokenType token;
  ...
  token = g_scanner_get_next_token(scanner);
  if(token != G_TOKEN_FLOAT) return G_TOKEN_FLOAT;
...
static GTokenType parse_genuine_float(GScanner *scanner, gdouble *result)
{
  ...
  else return G_TOKEN_FLOAT;
}

static GTokenType ifsvals_parse_element(GScanner *scanner, AffElementVals *result)
{
  GTokenType token;
  ...
  while(token != G_TOKEN_RIGHT_CURLY)
  {
    ...
    case TOKEN_X:
      ...
      if(expected_token != G_TOKEN_NONE) return expected_token;
      break;
    case TOKEN_Y:
 ...
 *************************************************************/
static gboolean ifsvals_parse(GScanner *scanner, IfsComposeVals *vals, AffElement ***elements)
{
  ...
  while(expected_token == G_TOKEN_NONE)
  {
    ...
    case TOKEN_ITERATIONS:
      token = g_scanner_get_next_token(scanner);
      if(token == G_TOKEN_INT) new_vals.iterations = scanner->value.v_int;
      else expected_token = G_TOKEN_INT;
      break;
    case TOKEN_MAX_MEMORY:
      token = g_scanner_get_next_token(scanner);
      if(token == G_TOKEN_INT) new_vals.max_memory = scanner->value.v_int;
      else expected_token = G_TOKEN_INT;
      break;
    case TOKEN_SUBDIVIDE:
      token = g_scanner_get_next_token(scanner);
      if(token == G_TOKEN_INT) new_vals.subdivide = scanner->value.v_int;
      else expected_token = G_TOKEN_INT;
      break;
    case TOKEN_RADIUS:
...
gboolean ifsvals_parse_string(const gchar *str, IfsComposeVals *vals, AffElement ***elements)
{
...
gchar* ifsvals_stringify(IfsComposeVals *vals, AffElement **elements)
{
  gint     i;
