...
static gint add_merged_image(const gint32 image_id, PSDimage *img_a, FILE *f, GError **error)
{
  ...
  if((img_a->color_mode == PSD_BITMAP || img_a->color_mode == PSD_GRAYSCALE ||
    img_a->color_mode == PSD_DUOTONE || img_a->color_mode == PSD_INDEXED) && total_channels > 1)
  {
    extra_channels = total_channels - 1;
  }
  else if((img_a->color_mode == PSD_RGB || img_a->color_mode == PSD_LAB) && total_channels > 3)
  {
    extra_channels = total_channels - 3;
  }
  else if((img_a->color_mode == PSD_CMYK) && total_channels > 4)
  {
    extra_channels = total_channels - 4;
  }
  if(img_a->transparency && extra_channels > 0) extra_channels--;
  base_channels = total_channels - extra_channels;

  // Read merged image & extra channel pixel data
  if(img_a->num_layers == 0 || extra_channels > 0)
  {
    ...
    case PSD_COMP_RAW: // Planar raw data
      ...
      for(cidx = 0; cidx < total_channels; ++cidx)
      {
        ...
        if(read_channel_data(&chn_a[cidx], img_a->bps, PSD_COMP_RAW, NULL, f, error) < 1)
          return -1;
      }
      break;
    case PSD_COMP_RLE: // Packbits
      ...
      IFDBG(3) g_debug("RLE length data: %d, RLE data block: %d",
        total_channels*img_a->rows*2, block_len - (total_channels*img_a->rows*2));
      for(cidx = 0; cidx < total_channels; ++cidx)
      {
        chn_a[cidx].columns = img_a->columns;
        chn_a[cidx].rows = img_a->rows;
      ...
      for(cidx = 0; cidx < total_channels; ++cidx)
      {
        if(read_channel_data(&chn_a[cidx], img_a->bps, PSD_COMP_RLE, rle_pack_len[cidx], f, error) < 1)
          return -1;
    ...
    case PSD_COMP_ZIP: // ?
    case PSD_COMP_ZIP_PRED:
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("Unsupported compression mode: %d"), comp_mode);
      return -1;
      break;
    }
  }
  ...
  if(img_a->num_layers == 0) // Merged image - Photoshop 2 style
  {
    ...
    layer_id = gimp_layer_new(image_id, _("Background"), img_a->columns, img_a->rows,
      image_type, 100, GIMP_NORMAL_MODE);
    gimp_image_add_layer(image_id, layer_id, 0);
    drawable = gimp_drawable_get(layer_id);
    gimp_pixel_rgn_init(&pixel_rgn, drawable, 0, 0, drawable->width, drawable->height, TRUE, FALSE);
    gimp_pixel_rgn_set_rect(&pixel_rgn, pixels, 0, 0, drawable->width, drawable->height);
    gimp_drawable_flush(drawable);
    gimp_drawable_detach(drawable);
    g_free(pixels);
  }
    ...
    && image_id > -1)
  {
    ...
    if(img_a->transparency)
    {
      ...
      if(img_a->alpha_names)
      {
        ...
        if(alpha_name) g_free(alpha_name);
      }
    }
    else offset = 0;
    ...
    for(i = 0; i < extra_channels; ++i)
    {
      ...
      if(!alpha_name && img_a->alpha_names)
        if(offset < img_a->alpha_names->len && i + offset <= img_a->alpha_names->len)
          alpha_name = g_ptr_array_index(img_a->alpha_names, i + offset);
      if(!alpha_name) alpha_name = g_strdup(_("Extra"));

      if(offset < img_a->alpha_id_count && offset + i <= img_a->alpha_id_count)
        alpha_id = img_a->alpha_id[i + offset];
      else alpha_id = 0;
      if(offset < img_a->alpha_display_count && i + offset <= img_a->alpha_display_count)
      {
      ...
      channel_id = gimp_channel_new(image_id, alpha_name, chn_a[cidx].columns, chn_a[cidx].rows,
        alpha_opacity, &alpha_rgb);
      gimp_image_add_channel(image_id, channel_id, 0);
      g_free(alpha_name);
      drawable = gimp_drawable_get(channel_id);
      if(alpha_id) gimp_drawable_set_tattoo(drawable->drawable_id, alpha_id);
    ...
    g_free(pixels);
    if(img_a->alpha_names) g_ptr_array_free(img_a->alpha_names, TRUE);
    if(img_a->alpha_id) g_free(img_a->alpha_id);
  ...
  if(img_a->layer_state + 1 > lyr_count || img_a->layer_state + 1 < 0)
    img_a->layer_state = 0;
