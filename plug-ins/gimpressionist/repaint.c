...
static int choose_best_brush(ppm_t *p, ppm_t *a, int tx, int ty,
  ppm_t *brushes, int num_brushes, double *brushes_sum, int start, int step)
{
  double dev, thissum;
  double bestdev = 0.0;
  double r, g, b;
...
static void apply_brush(ppm_t *brush, ppm_t *shadow, ppm_t *p, ppm_t *a,
...
void repaint(ppm_t *p, ppm_t *a)
{
  ...
  if(runningvals.place_type == PLACEMENT_TYPE_EVEN_DIST) density /= 3.0;
  ...
  if(dropshadow) shadows = g_malloc(num_brushes*sizeof(ppm_t));
  else shadows = NULL;
  ...
  if(bgamma != 1.0) ppm_apply_gamma(&brushes[0], 1.0/bgamma, 1, 1, 1);

  resize(&brushes[0], brushes[0].width*scale, brushes[0].height*scale);
  i = 1 + sqrt(brushes[0].width*brushes[0].width + brushes[0].height*brushes[0].height);
  ppm_pad(&brushes[0], i - brushes[0].width, i - brushes[0].width, i - brushes[0].height, i - brushes[0].height, back);
  ...
  {
    double sv;
    if(runningvals.size_num > 1) sv = i/(runningvals.size_num - 1.0);
    else sv = 1.0;
    for(j = 0; j < runningvals.orient_num; j++)
    {
      h = j + i*runningvals.orient_num;
      free_rotate(&brushes[h], startangle + j*anglespan/runningvals.orient_num);
  ...
  for(i = 0; i < num_brushes; i++)
  {
    if(!runningvals.color_brushes) prepare_brush(&brushes[i]);
  ...
  for(i = 0; i < num_brushes; i++)
  {
    ...
    if(xp || yp) ppm_pad(&brushes[i], xp/2, xp - xp/2, yp/2, yp - yp/2, blk);
  }
  ...
  if(dropshadow)
  {
    for(i = 0; i < num_brushes; i++)
    {
      ...
      ppm_pad(&shadows[i], shadowblur*2, shadowblur*2, shadowblur*2, shadowblur*2, back);
      for(j = 0; j < shadowblur; j++) blur(&shadows[i], 2, 2);
#ifdef REPAINT_DEBUG
  ...
  if(runningvals.general_paint_edges)
  {
    ...
    if(img_has_alpha) edgepad(a, maxbrushwidth, maxbrushwidth, maxbrushheight, maxbrushheight);
  }
  ...
  else
  {
    ...
    if(runningvals.paper_invert) ppm_apply_gamma(&paper_ppm, -1.0, 1, 1, 1);
  ...
  {
    i = (int)(tmp.width*density/maxbrushwidth)*(int)(tmp.height*density/maxbrushheight);
#ifdef REPAINT_DEBUG
    step = i;
  ...
  for(; i; i--)
  {
    if(i % progstep == 0)
    {
      ...
      {
        char tmps[40];
        g_snprintf(tmps, sizeof(tmps), "%.1f %%", 100*(1.0 - ((double)i/max_progress)));
    ...
    if(runningvals.place_type == PLACEMENT_TYPE_RANDOM)
    {
      tx = g_rand_int_range(random_generator, maxbrushwidth/2, tmp.width - maxbrushwidth/2);
      ty = g_rand_int_range(random_generator, maxbrushheight/2, tmp.height - maxbrushheight/2);
    }
    else if(runningvals.place_type == PLACEMENT_TYPE_EVEN_DIST)
    {
      tx = xpos[i - 1];
    ...
    if((tx < maxbrushwidth/2) || (ty < maxbrushwidth/2) ||
      (tx + maxbrushwidth/2 >= p->width) || (ty + maxbrushheight/2 >= p->height))
    {
#ifdef REPAINT_DEBUG
    ...
    if(img_has_alpha)
    {
      if(a->col[ty*a->width*3 + tx*3] > 128) continue;
    }
    ...
    if(runningvals.orient_type == ORIENTATION_ADAPTIVE)
    {
      if(runningvals.size_type == SIZE_TYPE_ADAPTIVE)
        n = choose_best_brush(p, a, tx - maxbrushwidth/2, ty - maxbrushheight/2, brushes,
          num_brushes, brushes_sum, 0, 1);
      else
      {
        int st = sn*runningvals.orient_num;
        n = choose_best_brush(p, a, tx - maxbrushwidth/2, ty - maxbrushheight/2, brushes,
          st + runningvals.orient_num, brushes_sum, st, 1);
      }
    }
    else
    {
      if(runningvals.size_type == SIZE_TYPE_ADAPTIVE)
        n = choose_best_brush(p, a, tx - maxbrushwidth/2, ty - maxbrushheight/2, brushes,
          num_brushes, brushes_sum, on, runningvals.orient_num);
      else n = sn*runningvals.orient_num + on;
    }
    ...
    brush = &brushes[n];
    if(dropshadow) shadow = &shadows[n];
...
#undef MYASSIGN
  ...
  if(relief > 0.001)
  {
    ...
    else
    {
      ...
      if(runningvals.paper_invert) ppm_apply_gamma(&tmp, -1.0, 1, 1, 1);
    }
    for(x = 0; x < p->width; x++)
    {
      double h, v;
      ...
      for(y = 0; y < p->height; y++)
      {
        ...
        if(runningvals.paper_overlay)
          ...
          h = -(tmp.col[py*tmp.width*3 + px*3] -
            (int)tmp.col[((py + 1) % tmp.height)*tmp.width*3 + ((px + 1) % tmp.width)*3])/2.0*relief;
        if(h <= 0.0)
        {
          v = 1.0 + h/128.0;
          if(v < 0.0) v = 0.0;
